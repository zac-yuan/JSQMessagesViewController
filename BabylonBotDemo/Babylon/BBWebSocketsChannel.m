...
@implementation BBWebSocketsChannel {
    NSString *_channelId;
    NSString *_authKey;
...
- (instancetype)initWithName:(NSString *)name heartbeat:(BOOL)heartbeat {
    ...
    if (self) {
        ...
        _clientUuid = [[NSUUID UUID] UUIDString];
        _heartbeat = heartbeat;
        _connectQueue = dispatch_queue_create("BBWebSocketsChannelConnectQueue", DISPATCH_QUEUE_CONCURRENT);
        _pollingQueue = dispatch_queue_create("BBWebSocketsChannelPollingQueue", DISPATCH_QUEUE_SERIAL);
        _immediateQueue = dispatch_queue_create("BBWebSocketsChannelImmediateQueue", DISPATCH_QUEUE_SERIAL);
        _heartbeatQueue = dispatch_queue_create("BBWebSocketsChannelHeartbeatQueue", DISPATCH_QUEUE_SERIAL);
    }
    return self;
}
...
- (void)subscribeWithUserInfo:(NSDictionary *)userInfo completion:(void (^)())completion {
    dispatch_barrier_async(_connectQueue, ^{
        if (!self.connected) {
            ...
            [self _dispatchChangeState:BBWebSocketsChannelStateConnected];
...
- (void)unsubscribeWithCompletion:(void (^)())completion {
    dispatch_barrier_async(_connectQueue, ^{
        if (self.connected) {
            ...
            _reachability = nil;
            [self _dispatchChangeState:BBWebSocketsChannelStateDisconnected];
...
- (void)_dispatchChangeState:(BBWebSocketsChannelState)newState {
    dispatch_async(dispatch_get_main_queue(), ^{
        // Disallow transition to/from disconnected <-> transientfailure.
        if (newState == _state || (newState == BBWebSocketsChannelStateTransientFailure && _state == BBWebSocketsChannelStateDisconnected) || (newState == BBWebSocketsChannelStateDisconnected && _state == BBWebSocketsChannelStateTransientFailure)) {
            return;
        }
        _state = newState;
        [self.delegate webSocketsChannel:self state:_state];
    });
}
...
- (void)_deliverMessages:(NSArray*)messages receivedDate:(NSDate*)receivedData {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self.delegate) {
            if (self.connected) {
                [self.delegate webSocketsChannel:self messages:messages receivedDate:receivedData];
            }
        }
    });
}
...
- (void)_processHeartbeatResponse:(NSObject *)object error:(NSError *)error {
    if (error) {
        ...
        if (_heartbeatFailureCount >= kPubNubHeartbeatFailureThreshold) {
            ...
            [self _dispatchChangeState:BBWebSocketsChannelStateTransientFailure];
        }
    } else {
        if (_heartbeatFailureCount >= kPubNubHeartbeatFailureThreshold) {
            // Reset the polling session in case it's still stuck in a long timeout.
            [self _resetPollingSession];
        }
        _heartbeatFailureCount = 0;
        [self _dispatchChangeState:BBWebSocketsChannelStateConnected];
