...
@implementation BBDotActivityIndicatorView {
    BOOL isAnimating;
}
...
- (void)startAnimating {
    if (!isAnimating) {
        if (self.dotParms == nil) {
            self.dotParms = [[BBDotActivityIndicatorParms alloc] initWithDefaultData];
        }
...
- (void)createCircleWithIndex:(NSUInteger)index {
    ...
    CGFloat origonY = self.dotParms.activityViewHeight/1.8 - self.dotParms.circleWidth/2;
